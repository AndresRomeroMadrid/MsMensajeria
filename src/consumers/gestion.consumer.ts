...
export const initGestionConsumer = async (): Promise<void> => {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      console.log(`[RabbitMQ] Intento ${attempt}/${MAX_RETRIES}...`);
      connection = await amqplib.connect(rabbitmqUrl);
      console.log('[RabbitMQ] ✅ Conexión establecida.');
      break;
    } catch (err: any) {
      console.error(`[RabbitMQ] ❌ Intento ${attempt} fallido: ${err.message}`);
      if (attempt < MAX_RETRIES) {
        console.log(`[RabbitMQ] Reintentando en ${RETRY_DELAY_MS / 1000}s...`);
        await wait(RETRY_DELAY_MS);
      } else {
        console.error('[RabbitMQ] ❌ Se agotaron los reintentos. El consumidor no se iniciará.');
        return;
      }
    }
  }
